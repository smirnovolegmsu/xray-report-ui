#!/bin/bash
# Скрипт для оптимизации портов - безопасное закрытие неиспользуемых портов

echo "=========================================="
echo "  ОПТИМИЗАЦИЯ ПОРТОВ НА СЕРВЕРЕ"
echo "=========================================="
echo ""
...
if [ "$CURSOR_COUNT" -gt 0 ]; then
    echo "💻 Найдено портов Cursor Server: $CURSOR_COUNT"
    echo "   Эти порты безопасны (слушают только localhost)"
    echo "   Закрывайте только если не используете Cursor через SSH"
    echo ""
fi
