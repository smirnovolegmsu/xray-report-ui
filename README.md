...
│       └── js/*.js             - 12 модулей JavaScript
│
├── 🐍 BACKEND (логика на сервере)
│   ├── app.py                  - Flask сервер
│   ├── config.py               - Конфигурация
│   ├── core/                   - Бизнес-логика
│   │   ├── events.py          - Логирование
│   │   ├── backup.py          - Бэкапы
...
2. `app.py` → вызывает `core/xray.py`
3. `core/xray.py` → читает config, добавляет юзера, делает бэкап
4. Возвращает `{ok: true}`
5. Браузер обновляет таблицу
...
- **Backend** (`app.py`, `core/`, `utils/`) - Python логика
- **Данные** (`data/`) - не добавляется в Git

### Зависимости модулей:

```
app.py → config.py, core/*, utils/*
...
## 📝 TODO (Backend рефакторинг)

- [ ] `core/xray.py` - Работа с Xray конфигом
- [ ] `core/usage.py` - Парсинг CSV статистики
- [ ] `core/live_monitor.py` - Live мониторинг
- [ ] `api/users.py` - API пользователей
- [ ] `api/dashboard.py` - API статистики
- [ ] `api/live.py` - API real-time
- [ ] `api/system.py` - API системы
- [ ] Тесты (pytest)

## 🎉 Итоги

**Frontend:** ✅ Завершен (модульный, быстрый)  
**Backend:** 🔄 30% (базовая структура готова)  
