...
# Алиасы лучше хранить рядом с UI (а не в /var/log)
...
ADMIN_TOKEN = os.getenv("XRAY_UI_ADMIN_TOKEN", "").strip()  # если пусто — без токена
...
def _find_vless_inbound(cfg: Dict[str, Any]) -> Tuple[Optional[Dict[str, Any]], Optional[List[Dict[str, Any]]]]:
    ...
    for ib in inbounds:
        ...
        if isinstance(clients, list):
            return ib, clients
    # fallback: первый inbound где есть clients
    for ib in inbounds:
        if not isinstance(ib, dict):
            continue
        settings = ib.get("settings") or {}
...
def _restart_xray() -> Dict[str, Any]:
    # применить add/delete клиентов без рестарта — не всегда работает, поэтому рестарт
    try:
        p = subprocess.run(
...
def _detect_public_ip() -> str:
    # на VPS обычно вернет публичный IP интерфейса
    try:
...
def _derive_reality_public_key_b64(private_key_b64: str) -> Optional[str]:
    # Xray realitySettings.privateKey обычно base64 (32 bytes)
    try:
...
def _build_vless_reality_link(cfg: Dict[str, Any], email: str, cid: str) -> Dict[str, Any]:
    ...
    port = None
    if ib and isinstance(ib.get("port"), int):
        port = ib.get("port")
    if port is None:
        try:
    ...
    # даже если pbk/sni/sid пустые — вернём, но это будет "неполный" линк
...
def _aggregate_last_n(days: List[str]) -> Dict[str, Any]:
    ...
    # report per-domain traffic + daily sums (priority)
    for day in days:
...
def _alltime_stats() -> Dict[str, Any]:
    ...
    # собрать пользователей даже если нет all-time файлов
...
def api_admin_user_add():
    ...
    # flow по образцу первого клиента, иначе дефолт
    flow = ""
    for c in clients:
