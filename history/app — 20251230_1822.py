...
UI_PATH = os.getenv("XRAY_UI_INDEX", str(Path(__file__).resolve().with_name("index.html")))
...
def _try_load_ip_domain_map() -> Dict[str, str]:
    """
    domains_YYYY-MM-DD.csv должен быть: ip,domain
    Если формат другой — игнорируем.
    """
    dates = _scan_dates()
    if not dates:
        return {}
...
# ---------------------------
# Aggregation (важно: НЕ удваиваем трафик)
# ---------------------------
...
def _aggregate(days: List[str]) -> Dict[str, Any]:
    ...
    # report: domain traffic + sums (has priority over usage if >0)
    for day in days:
    ...
    # override daily totals with report sums if any
...
# ---- Compatibility endpoints (если вдруг старый UI ещё дергает) ----
